...
public interface IUniWebViewAuthenticationFlow<TTokenType>
{
    ...
    /// <summary>
    /// Called when Sign Out flow is successful
    /// </summary>
    UnityEvent<string> OnSignOutFinished { get; set; }

    /// <summary>
    /// Called when any error happens during Sign Out flow
    /// </summary>
    UnityEvent<string> OnSignOutErrored { get; set; }
}
...
public class UniWebViewAuthenticationFlow<TTokenType> {
    ...
    {
        var callbackUri = new Uri(service.GetCallbackUrl());
        var revokeUrl = GetRevokeUrl();
        var session = UniWebViewAuthenticationSession.Create(revokeUrl, callbackUri.Scheme);
        var flow = service as UniWebViewAuthenticationCommonFlow;
        if (flow != null && flow.privateMode)
        {
            session.SetPrivateMode(true);
        ...
        session.OnAuthenticationFinished += (_, resultUrl) => {
            UniWebViewLogger.Instance.Verbose("Revoke flow received callback url: " + resultUrl);
            if (service.OnSignOutFinished != null)
            {
                service.OnSignOutFinished.Invoke("success");
            }
        };

        session.OnAuthenticationErrorReceived += (_, errorCode, message) => {
            UniWebViewLogger.Instance.Verbose("Revoke flow errored: " + message);
            if (service.OnSignOutErrored != null)
            {
                service.OnSignOutErrored.Invoke(errorCode + " " + message);
            }
        };

        UniWebViewLogger.Instance.Verbose("Starting revoke flow with url: " + revokeUrl + "; Callback scheme: " + callbackUri.Scheme);
        session.Start();
    }
    ...
    {
        UniWebViewLogger.Instance.Debug(service.GetAuthenticationConfiguration().revokeEndpoint);
        var builder = new UriBuilder(service.GetAuthenticationConfiguration().revokeEndpoint);
        var query = new Dictionary<string, string>();
        query.Add("client_id", service.GetAuthenticationUriArguments()["client_id"]);
        query.Add("logout_uri", service.GetAuthenticationUriArguments()["redirect_uri"]);
        builder.Query = UniWebViewAuthenticationUtils.CreateQueryString(query);
        return builder.ToString();
    }
...
public class UniWebViewAuthenticationConfiguration {
    ...
    internal readonly string revokeEndpoint;
    ...
    public UniWebViewAuthenticationConfiguration(string authorizationEndpoint, string tokenEndpoint, string revokeEndpoint)
    {
