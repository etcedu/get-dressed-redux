...
public class UniWebViewAuthenticationFlowCustomize : UniWebViewAuthenticationCommonFlow, IUniWebViewAuthenticationFlow<UniWebViewAuthenticationStandardToken> {
    ...
    //WTP NOTE: I switched out the original redirectUri (now mobileRedirectUri) with this property in order to more easily support windows redirect being different
    ...
    //WTP NOTE: I added this windows specific uri to make it easier on our developers
    public string windowsRedirectUri = "http://localhost:8080/simcoachauth/";
    ...
    {
        // Debug.Log("StartAuthenticationFlow()");
        var flow = new UniWebViewAuthenticationFlow<UniWebViewAuthenticationStandardToken>(this);
        flow.StartAuth();
    }

    /// <summary>
    /// Starts auth flow but with /logout endpoint
    /// </summary>
    public void StartRevokeFlow()
    {
        // Debug.Log("StartRevokeFlow()");
        var flow = new UniWebViewAuthenticationFlow<UniWebViewAuthenticationStandardToken>(this);
        flow.StartRevoke();
    }
    ...
    /// </summary>
    [field: SerializeField]
    public UnityEvent<string> OnSignOutFinished { get; set; }
