...
public abstract class UITweener : MonoBehaviour
{
	...
	[HideInInspector] public bool triggerOnFinishedOnlyOnForward;
	[HideInInspector] public List<EventDelegate> onFinished = new List<EventDelegate>();
    [HideInInspector] public List<EventDelegate> onStarted = new List<EventDelegate>();
	...
	bool mStarted = false;
	bool mTriggedStartEvents = false;
	float mStartTime = 0f;
	float mDuration = 0f;
	...
	protected void DoUpdate ()
	{
		...
		if (!mTriggedStartEvents)
		{
			mTriggedStartEvents = true;
			if (current != this)
			{
				...
				{
					mTemp = onStarted;
					onStarted = new List<EventDelegate>();
					...
					for (int i = 0; i < mTemp.Count; ++i)
					{
						...
						if (ed != null && !ed.oneShot) EventDelegate.Add(onStarted, ed, ed.oneShot);
					}
					mTemp = null;
				}
				current = before;
		...
		if ((style == Style.Once) && (duration == 0f || mFactor > 1f || mFactor < 0f))
		{
			...
			if (current != this)
			{
				...
				if (onFinished != null)
				{
					...
					// Notify the listener delegates
					if (!triggerOnFinishedOnlyOnForward)
						EventDelegate.Execute(mTemp);
					else if (mFactor >= 1f)
                        EventDelegate.Execute(mTemp);
	...
	void OnDisable () { mStarted = false; mTriggedStartEvents = false;
    }
    ...
    public void PauseForward_FromBeginning()
    {
        SetForward();
        ResetToBeginning();
        Pause();
    ...
    public void PauseReverse_FromBeginning()
    {
        SetReverse();
        ResetToBeginning();
        Pause();
...
#endif
		comp.mStarted = false;
		comp.mTriggedStartEvents = false;
		comp.mFactor = 0f;
