...
namespace UnityEngine.EventSystems
{
    ...
    public class EventSystem : UIBehaviour
    {
        ...
        bool lastInteractionWasWithLimitedObject;
        public bool CanInteractWith(GameObject currentOverGo, bool checkChildren = true)
        {
            lastInteractionWasWithLimitedObject = false;

            if (currentOverGo == null || limitInputToThisObject == null)
                return true;

            foreach (GameObject go in limitInputExceptionList)
            {
            ...
            if (checkChildren)
            {
                foreach (Transform child in limitInputToThisObject.transform.GetComponentsInChildren<Transform>())
                {
                    if (child.gameObject == currentOverGo)
                        canInteract = true;
                }
            ...
            lastInteractionWasWithLimitedObject = true;
            return canInteract;
        }

        public void CheckLimitedInteractionCompleted()
        {
            if (lastInteractionWasWithLimitedObject)
            {
                eventOnLimitedInputInteraction?.Invoke();
